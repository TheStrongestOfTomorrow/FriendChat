  ...
  return (
    <div className="flex flex-col h-screen max-h-screen bg-whatsapp-bg">
      <header className="bg-whatsapp-darkGreen text-white px-4 py-3 flex items-center gap-3 shadow-md shrink-0 z-20">
        <button onClick={onLeave} className="p-2 hover:bg-white/10 rounded-full"><ArrowLeft size={24}/></button>
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            <div className="w-12 h-12 bg-whatsapp-green rounded-full flex items-center justify-center text-white font-black text-base shadow-inner border-2 border-white/20">
              {room.name[0].toUpperCase()}
            </div>
            <div className="truncate">
                <h2 className="text-lg font-black truncate uppercase tracking-tight">{room.name}</h2>
                <div className="flex items-center gap-2">
                    <span className="w-2.5 h-2.5 bg-green-400 rounded-full animate-pulse"></span>
                    <span className="text-base font-black uppercase tracking-widest opacity-90">Online Group</span>
                </div>
            </div>
          </div>
        </div>
        <div className="flex items-center gap-1">
          ...
          }} className="p-3 hover:bg-white/10 rounded-full transition-colors"><Share2 size={20}/></button>
          <button onClick={() => onToggleVoice(room.id)} className={`p-3 rounded-full transition-all ${localStream ? 'bg-red-500 text-white animate-pulse' : 'hover:bg-white/10'}`}><Phone size={20}/></button>
          <div className="relative">
              <button onClick={() => setShowStatusMenu(!showStatusMenu)} className="p-3 hover:bg-white/10 rounded-full"><MoreVertical size={20}/></button>
              {showStatusMenu && (
                  <div className="absolute right-0 top-full mt-2 bg-white rounded-2xl shadow-2xl py-2 w-64 text-black z-50 animate-in fade-in zoom-in duration-200 border border-gray-100">
                      ...
                      <button onClick={() => { saveSpaceBlueprint({ id: room.id, name: room.name, originalHostId: room.originalHostId, inviteCode: room.hostPeerId, createdAt: Date.now() }); alert("Group Saved!"); }} className="w-full px-4 py-4 text-left flex items-center gap-3 hover:bg-gray-50 transition-colors font-black uppercase tracking-widest text-base">
                          <Save size={18}/> Save Group
                      </button>
                      {(room.hostPeerId === peerId || managerId === peerId) && (
                          <button onClick={onStopRoom} className="w-full px-4 py-4 text-left flex items-center gap-3 text-red-500 hover:bg-red-50 transition-colors font-black uppercase tracking-widest text-base">
                              <Power size={18}/> Close Room
                          </button>
                      )}
                      <button onClick={onLeave} className="w-full px-4 py-4 text-left flex items-center gap-3 hover:bg-gray-50 transition-colors font-black uppercase tracking-widest text-base">
      ...
      <div className="flex-1 flex overflow-hidden relative">
            ...
            <div className="flex-1 flex overflow-hidden">
                <aside className="w-[380px] hidden md:flex flex-col bg-white border-r border-gray-100 shadow-xl z-10">
                    <div className="p-6 bg-gray-50 border-b border-gray-100">
                        <h3 className="text-lg font-black uppercase tracking-[0.2em] text-whatsapp-darkGreen flex items-center gap-2">
                            <Users size={18}/> Friends ({users.length})
                        </h3>
                    </div>
                    <div className="flex-1 overflow-y-auto scrollbar-hide">
                        ...
                        >
                            ...
                            <div className="text-left">
                                <p className="font-black text-lg uppercase tracking-tight">Group Chat</p>
                                <p className="text-base font-black text-whatsapp-darkGreen uppercase tracking-widest">Everyone</p>
                            </div>
                        </button>
                        ...
                        {users.filter(u => u.peerId !== peerId).map(u => (
                            ...
                            >
                                <div className="relative">
                                    <div className="w-16 h-16 bg-gray-200 rounded-2xl flex items-center justify-center text-gray-500 font-black text-base border-2 border-white shadow-md">
                                ...
                                <div className="text-left flex-1">
                                    ...
                                    <p className="text-base font-black text-gray-900 uppercase tracking-widest opacity-100">{u.status}</p>
                                </div>
                                {(room.hostPeerId === peerId || managerId === peerId) && (
                                    <button
                                        onClick={(e) => {
                ...
                <main className="flex-1 flex flex-col bg-[#e5ddd5] relative">
                    {selectedUser && (
                        <div className="bg-white/90 backdrop-blur-md px-6 py-4 border-b border-black/5 flex items-center gap-4 z-10 shadow-sm animate-in slide-in-from-top duration-300">
                            <button onClick={() => setSelectedUser(null)} className="md:hidden p-2 text-whatsapp-darkGreen"><ArrowLeft size={24}/></button>
                            <span className="font-black text-black text-xl">{selectedUser.name}</span>
                            <span className="ml-auto text-lg text-whatsapp-darkGreen font-black bg-whatsapp-green/10 px-3 py-1 rounded-full border border-whatsapp-green/20 uppercase tracking-tighter">Private Chat</span>
                        </div>
                    )}
                    ...
                    <div ref={scrollRef} className="chat-scroll p-4 flex flex-col flex-1 overflow-y-auto overflow-x-hidden scroll-smooth">
                        ...
                        {typingUsers.size > 0 && Array.from(typingUsers).filter(u => u !== userName).length > 0 && (
                            <div className="text-lg italic text-black bg-white/80 px-4 py-2 rounded-full w-fit self-start mb-4 shadow-md border border-black/5 animate-pulse font-black">
                    ...
                    <footer className="bg-whatsapp-gray p-4 flex items-center gap-3 shrink-0 pb-[max(16px,env(safe-area-inset-bottom))] border-t border-black/5 z-10 shadow-lg">
                        <button onClick={() => fileInputRef.current?.click()} className="text-black p-3 hover:bg-gray-200 rounded-full transition-colors"><Paperclip size={24} /></button>
                        ...
                        <form onSubmit={handleSend} className="flex-1">
                            <input
                                ...
                                className="w-full bg-white rounded-full px-8 py-4 text-lg shadow-inner border-none focus:ring-4 focus:ring-whatsapp-green/20 font-black"
                            />
                        </form>
                        {input.trim() ? (
                            <button onClick={handleSend} className="bg-whatsapp-green text-white p-4 rounded-full shadow-2xl active:scale-95 transition-all"><Send size={24} /></button>
                        ) : (
        ...
        {activeTab === 'WALL' && (
            <div className="flex-1 flex flex-col bg-white animate-in slide-in-from-right duration-300">
                <div className="p-4 border-b border-gray-100 bg-gray-50 sticky top-0 z-10">
                    <h3 className="font-black text-whatsapp-darkGreen text-center uppercase tracking-[0.2em] leading-none text-xl">The Wall</h3>
                    <p className="text-lg text-gray-900 text-center uppercase font-black mt-1">Class Board</p>
                </div>
                <div className="flex-1 overflow-y-auto p-6 space-y-8 scrollbar-hide bg-gray-50 shadow-inner">
                    {wallPosts.length === 0 ? (
                        <div className="flex flex-col items-center justify-center h-full text-center opacity-100">
                            <Info size={32} className="mb-6 text-whatsapp-darkGreen" />
                            <p className="font-black uppercase tracking-widest text-lg">Nothing here yet</p>
                        </div>
                    ) : wallPosts.map(post => (
                        <div key={post.id} className="bg-white p-6 rounded-3xl border-l-[12px] border-whatsapp-teal shadow-xl animate-in fade-in duration-500">
                            <div className="flex justify-between items-start mb-4">
                                <span className="font-black text-whatsapp-darkGreen text-xl uppercase tracking-wider">{post.senderName}</span>
                                <span className="text-base text-gray-900 font-black">{new Date(post.timestamp).toLocaleString()}</span>
                            </div>
                            {post.type === 'image' ? (
                                <img src={post.content} className="rounded-2xl w-full border border-black/5 shadow-md" alt="Post" />
                            ) : (
                                <p className="text-black text-xl whitespace-pre-wrap leading-relaxed font-black">{post.content}</p>
                            )}
                        </div>
                    ))}
                </div>
                    ...
                    <div className="max-w-4xl mx-auto flex items-center gap-4">
                        ...
                        <form onSubmit={handleWallPost} className="flex-1 flex gap-4">
                            <input
                                ...
                                className="flex-1 bg-white border border-gray-200 rounded-2xl px-6 py-4 text-lg shadow-inner focus:ring-4 focus:ring-whatsapp-green/10 font-black"
                            />
                            <button type="submit" className="bg-whatsapp-green text-white p-4 rounded-2xl shadow-xl active:scale-95 transition-all"><SendHorizontal size={24}/></button>
                        </form>
                    </div>
                </footer>
            </div>
        ...
        {activeTab === 'CALL' && (
            <div className="flex-1 flex flex-col items-center justify-center p-8 bg-whatsapp-darkGreen text-white text-center animate-in zoom-in duration-300">
                ...
                <h3 className="text-lg font-black mb-4 uppercase tracking-tighter italic">Voice Call</h3>
                <p className="text-xl opacity-100 mb-12 max-w-sm leading-relaxed font-black">Everyone connects to everyone. Resilient calling.</p>
      ...
      {uploadProgress !== null && (
          <div className="fixed inset-0 bg-black/90 z-[100] flex items-center justify-center p-10 backdrop-blur-xl animate-in fade-in duration-300">
              <div className="bg-white rounded-[3rem] p-16 w-full max-w-md shadow-[0_0_100px_rgba(0,0,0,0.5)] text-center border-t-[16px] border-whatsapp-teal relative overflow-hidden">
                  <Activity size={32} className="mx-auto mb-8 text-whatsapp-darkGreen animate-bounce" />
                  <h4 className="font-black text-black mb-4 uppercase tracking-widest text-xl italic">Sending Data</h4>
                  <p className="text-lg text-gray-900 mb-10 font-black uppercase tracking-widest opacity-100 italic">Low-RAM Protocol Active</p>
                  ...
                  <p className="text-lg font-black text-whatsapp-darkGreen tracking-tighter">{`${Math.round(uploadProgress)}%`}</p>
                  <button onClick={() => window.location.reload()} className="mt-12 px-10 py-4 bg-red-50 text-red-500 rounded-full text-lg font-black uppercase tracking-widest border-2 border-red-100 shadow-md active:scale-95 hover:bg-red-100 transition-all">Cancel Transfer</button>
