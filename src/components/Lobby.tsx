...
import { ImageIcon as ImageIconLucide, RefreshCw, Lock, Plus, Search, MessageCircle, Hash, Copy, ChevronRight, Save, Zap, Activity, Trash2, Share2 } from 'lucide-react';
...
export const Lobby: React.FC<LobbyProps> = ({ onJoinRoom, peerId }) => {
  ...
  const handleClearHistory = async () => {
      if (window.confirm('PERMANENTLY_CLEAR_ALL_MESSAGE_HISTORY?')) {
          await clearMessages();
          localStorage.removeItem('saved-spaces');
          setSavedSpaces([]);
          alert('HISTORY_PURGED: Local database reset.');
      }
  };
  ...
  const copyInviteCode = (code: string) => {
      ...
      alert('LINK_COPIED: Shared to clipboard.');
  };
  ...
  return (
    <div className="min-h-screen bg-whatsapp-bg font-sans flex flex-col overflow-hidden animate-in fade-in duration-500">
          ...
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p className="text-[10px] opacity-60 uppercase tracking-[0.4em] font-black mt-1">Decentralized Mesh</p>
                </div>
            </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 md:p-8 space-y-12 pb-24 scrollbar-hide">
        {savedSpaces.length > 0 && (
            <section className="max-w-4xl mx-auto space-y-4 animate-in slide-in-from-bottom-4 duration-500 delay-100">
                <h2 className="text-xs font-black text-whatsapp-darkGreen uppercase tracking-[0.2em] flex items-center gap-2 opacity-70">
                    <Save size={14}/> Saved Architectures
                </h2>
                        ...
                        return (
                            <div key={space.id} className="bg-white p-6 rounded-3xl shadow-lg border-l-8 border-whatsapp-teal flex flex-col justify-between group hover:shadow-xl transition-all relative overflow-hidden">
                                <div className="relative z-10">
                                    ...
                                    <p className="text-[10px] text-gray-400 uppercase font-black tracking-widest">Creator ID: {space.originalHostId.slice(0, 12)}...</p>
                                </div>
                                <div className="mt-8 flex gap-3 relative z-10">
                                    {active ? (
                                        ...
                                        >
                                            <Zap size={14} fill="white"/> Access Live Mesh
                                        </button>
                                    ) : (
                                        <button
        ...
        <section className="max-w-4xl mx-auto space-y-6 animate-in slide-in-from-bottom-4 duration-500 delay-200">
            <div className="flex items-center justify-between">
                <h2 className="text-xs font-black text-whatsapp-darkGreen uppercase tracking-[0.2em] flex items-center gap-2 opacity-70">
                    <Zap size={14}/> Active Transmissions
                </h2>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {filteredRooms.length === 0 ? (
                    <div className="col-span-full py-16 text-center bg-white/50 backdrop-blur-sm rounded-3xl border-2 border-dashed border-gray-200">
                        ...
                        <p className="text-[10px] font-black text-gray-400 uppercase tracking-[0.3em]">Querying Decentralized Grid...</p>
                    </div>
                    ...
                    >
                        <div className="relative z-10">
                            ...
                            <p className="text-[10px] text-gray-400 uppercase font-black tracking-widest mt-1 opacity-70">Link: {room.hostPeerId.slice(0, 12)}...</p>
                        </div>
                ...
                >
                    <Plus size={40} className="mb-1" /> Establish Node
                </button>
            </div>
        </section>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-4xl mx-auto animate-in slide-in-from-bottom-4 duration-500 delay-300">
            <section className="bg-white p-10 rounded-3xl shadow-xl border-t-8 border-whatsapp-teal">
                <h2 className="font-black text-whatsapp-darkGreen mb-8 flex items-center gap-3 uppercase tracking-[0.2em] text-xs">
                    <Hash size={20} className="text-whatsapp-teal" /> Peer Access Link
                </h2>
                <form onSubmit={handleJoinByCode} className="space-y-6">
                    <input
                        type="text"
                        placeholder="PASTE_NODE_ID"
                        value={joinCode}
                    ...
                    >
                        {isSearchingCode ? 'Authenticating...' : 'Sync with Peer'}
                    </button>
                </form>
            </section>
            ...
            <div className="bg-whatsapp-darkGreen text-white p-10 rounded-3xl shadow-2xl flex flex-col justify-center text-center relative overflow-hidden group">
                <div className="relative z-10">
                    <p className="text-[10px] font-black uppercase tracking-[0.4em] mb-6 opacity-60">Global Mesh Identifier</p>
                    ...
                    <button onClick={() => { navigator.clipboard.writeText(peerId); alert('Code Copied!'); }} className="flex items-center justify-center gap-3 w-full bg-whatsapp-green text-black font-black py-5 rounded-2xl text-[10px] uppercase tracking-[0.4em] shadow-2xl hover:bg-white transition-all active:scale-95">
                        <Copy size={18}/> Broadcast Identity
                    </button>
                </div>
      ...
      {isCreating && (
          <div className="fixed inset-0 bg-black/70 backdrop-blur-md flex items-center justify-center p-6 z-50 animate-in fade-in duration-300">
              <form onSubmit={handleCreateRoom} className="bg-white rounded-[2rem] p-10 md:p-14 w-full max-w-lg shadow-2xl animate-in zoom-in duration-300 relative overflow-hidden border-t-8 border-whatsapp-green">
                  <div className="relative z-10">
                      <h2 className="text-4xl font-black text-whatsapp-darkGreen mb-2 uppercase tracking-tighter italic">New Network</h2>
                      <p className="text-[10px] text-gray-400 mb-12 font-black uppercase tracking-[0.3em]">Initialize a secure decentralized space</p>
                      <div className="space-y-8">
                          <div>
                              <label className="block text-[10px] font-black text-gray-400 uppercase tracking-[0.4em] mb-3 leading-none">Access Point Name</label>
                              <input
                          ...
                          <div>
                              <label className="block text-[10px] font-black text-gray-400 uppercase tracking-[0.4em] mb-3 leading-none">Security Key [OPTIONAL]</label>
                              <input
                      ...
                      <div className="flex gap-6 mt-14">
                          <button type="button" onClick={() => setIsCreating(false)} className="flex-1 py-5 text-gray-400 font-black uppercase tracking-[0.3em] text-[10px] hover:text-red-500 transition-colors">Abort</button>
                          <button type="submit" className="flex-1 py-5 bg-whatsapp-teal text-white font-black rounded-2xl shadow-2xl shadow-whatsapp-teal/20 uppercase tracking-[0.3em] text-[10px] active:scale-95 transition-all">Initialize</button>
                      </div>
                  </div>
